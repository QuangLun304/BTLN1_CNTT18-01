...
def them_lop():
    try:
        with open(data_patch, 'r', encoding='utf-8') as f:
            sinh_vien = json.load(f)
        ma_lop = input("Nhập mã buổi học: ").strip()
    except ValueError:
...
def them_sinh_vien():
    ...
    sinh_vien[ma_lop].update(sinh_vien_moi)
    with open(data_patch, 'w', encoding='utf-8') as f:
        json.dump(sinh_vien, f, ensure_ascii=False, indent=4)
...
def hien_thi_danh_sach_sinh_vien():
    ...
    print('-' * 141)
    for ma_lop, danh_sach in sinh_vien.items():
        for ma_sv, info in danh_sach.items():
            print(f"|{ma_lop:<10}|{ma_sv:<10}|{info['ho_ten']:<30}|{info['ngay_sinh']:<12}|{info['gioi_tinh']:<10}|{info['dia_chi']:<20}|{info['khoa']:<15}|{info['tinh_trang']:<12}|{info['so_tiet_nghi']:<12}|")
...
def hien_thi_danh_sach_sinh_vien_theo_ma_lop():
    with open(data_patch, 'r', encoding='utf-8') as f:
        sinh_vien = json.load(f)
    ma_lop = input("Nhập mã buổi học: ").strip()
    print(f"|=========================================================   Danh sách sinh viên   =========================================================|")
    print(f"|{'Mã Học':<10}|{'Mã SV':<10}|{'Họ tên':<30}|{'Ngày sinh':<12}|{'Giới tính':<10}|{'Địa chỉ':<20}|{'Khoa':<15}|{'Tình trạng':<12}|{'Số tiết nghỉ':<12}|")
    print('-' * 141)
    for ma_sv, info in sinh_vien[ma_lop].items():
        if ma_lop in sinh_vien:
                print(f"|{ma_lop:<10}|{ma_sv:<10}|{info['ho_ten']:<30}|{info['ngay_sinh']:<12}|{info['gioi_tinh']:<10}|{info['dia_chi']:<20}|{info['khoa']:<15}|{info['tinh_trang']:<12}|{info['so_tiet_nghi']:<12}|")
...
def tim_Kiem_sinh_vien_theo_ma_sinh_vien():
    ...
    print(f"{'Mã lớp':<8} {'Mã SV':<10} {'Họ tên':<30} {'Ngày sinh':<12} {'Giới tính':<10} {'Địa chỉ':<20} {'Khoa':<15} {'Tình trạng':<12} {'Số tiết nghỉ':<5}")
    print('-' * 140)
    for ma_lop, danh_sach_sv in sinh_vien.items():
        if ma_sv in danh_sach_sv:
            sv = danh_sach_sv[ma_sv]
            print(f"{ma_lop:<8} {ma_sv:<10} {sv['ho_ten']:<30} {sv['ngay_sinh']:<12} {sv['gioi_tinh']:<10} {sv['dia_chi']:<20} {sv['khoa']:<15} {sv['tinh_trang']:<12} {sv['so_tiet_nghi']:<5}")
            tim_thay = True
    if not tim_thay:
...
def tim_Kiem_sinh_vien_theo_ten():
    with open(data_patch, 'r', encoding='utf-8') as f:
        sinh_vien = json.load(f)
    ...
    print('-' * 140)
    for ma_lop, danh_sach_sv in sinh_vien.items():
        for ma_sv, sv in danh_sach_sv.items():
            if ho_ten in sv['ho_ten'].lower():
                print(f"{ma_lop:<8} {ma_sv:<10} {sv['ho_ten']:<30} {sv['ngay_sinh']:<12} {sv['gioi_tinh']:<10} {sv['dia_chi']:<20} {sv['khoa']:<15} {sv['tinh_trang']:<12} {sv['so_tiet_nghi']:<5}")
                tim_thay = True
    if not tim_thay:
        print(f"Không tìm thấy sinh viên nào có tên chứa: '{ho_ten}'")



def hien_thi_danh_sach_sinh_vien_theo_tinh_trang_cam_thi():
    with open(data_patch, 'r', encoding='utf-8') as f:
        sinh_vien = json.load(f)
    tim_thay = False
    print(f"=========================================   Danh sách sinh viên bị CẤM THI (nghỉ > 8 buổi)   =========================================")
    print(f"{'Mã lớp':<8} {'Mã SV':<10} {'Họ tên':<30} {'Ngày sinh':<12} {'Giới tính':<10} {'Địa chỉ':<20} {'Khoa':<15} {'Tình trạng':<12} {'Số tiết nghỉ':<5}")
    print('-' * 140)
    for ma_lop, danh_sach_sv in sinh_vien.items():
        for ma_sv, sv in danh_sach_sv.items():
            if sv['so_tiet_nghi'] > 8:
                print(f"{ma_lop:<8} {ma_sv:<10} {sv['ho_ten']:<30} {sv['ngay_sinh']:<12} {sv['gioi_tinh']:<10} {sv['dia_chi']:<20} {sv['khoa']:<15} {sv['tinh_trang']:<12} {sv['so_tiet_nghi']:<5}")
                tim_thay = True

    if not tim_thay:
        print("Không có sinh viên nào bị cấm thi (tất cả nghỉ <= 8 buổi)")


def hien_thi_sinh_vien_theo_so_tiet_nghi():
    with open(data_patch, 'r', encoding='utf-8') as f:
        sinh_vien = json.load(f)
    try:
        muc_nghi = int(input("Nhập số tiết nghỉ tối thiểu để lọc sinh viên: ").strip())
    except ValueError:
    ...
    for ma_lop, danh_sach_sv in sinh_vien.items():
        for ma_sv, sv in danh_sach_sv.items():
            if sv['so_tiet_nghi'] >= muc_nghi:
                print(f"{ma_lop:<8} {ma_sv:<10} {sv['ho_ten']:<30} {sv['ngay_sinh']:<12} {sv['gioi_tinh']:<10} {sv['dia_chi']:<20} {sv['khoa']:<15} {sv['tinh_trang']:<12} {sv['so_tiet_nghi']:<5}")
                tim_thay = True

    if not tim_thay:
        print(f"Không có sinh viên nào nghỉ từ {muc_nghi} tiết trở lên.")


def hien_thi_sinh_vien_di_hoc_day_du():
    with open(data_patch, 'r', encoding='utf-8') as f:
        sinh_vien = json.load(f)
    tim_thay = False
    print(f"=========================================   Danh sách sinh viên đi học đầy đủ (nghỉ = 0)   ==========================================")
    print(f"{'Mã lớp':<8} {'Mã SV':<10} {'Họ tên':<30} {'Ngày sinh':<12} {'Giới tính':<10} {'Địa chỉ':<20} {'Khoa':<15} {'Tình trạng':<12} {'Số tiết nghỉ':<5}")
    print('-' * 140)
    for ma_lop, danh_sach_sv in sinh_vien.items():
        for ma_sv, sv in danh_sach_sv.items():
            if sv['so_tiet_nghi'] == 0:
                print(f"{ma_lop:<8} {ma_sv:<10} {sv['ho_ten']:<30} {sv['ngay_sinh']:<12} {sv['gioi_tinh']:<10} {sv['dia_chi']:<20} {sv['khoa']:<15} {sv['tinh_trang']:<12} {sv['so_tiet_nghi']:<5}")
                tim_thay = True
    if not tim_thay:
...
def thong_ke_so_luong_theo_lop():
    ...
    for ma_lop, danh_sach_sv in du_lieu.items():
        so_luong = len(danh_sach_sv)
        print(f"Lớp {ma_lop}: {so_luong} sinh viên")



def tim_sinh_vien_theo_dia_chi():
    with open(data_patch, 'r', encoding='utf-8') as f:
        sinh_vien = json.load(f)
    dia_chi_can_tim = input("Nhập địa chỉ cần tìm: ").strip().lower()
    tim_thay = False
    print(f"=====================================   Danh sách sinh viên theo địa chỉ chứa: '{dia_chi_can_tim}'   =====================================")
    print(f"{'Mã lớp':<8} {'Mã SV':<10} {'Họ tên':<30} {'Ngày sinh':<12} {'Giới tính':<10} {'Địa chỉ':<20} {'Khoa':<15} {'Tình trạng':<12} {'Số tiết nghỉ':<5}")
    print('-' * 140)
    for ma_lop, danh_sach_sv in sinh_vien.items():
        for ma_sv, sv in danh_sach_sv.items():
            if dia_chi_can_tim in sv['dia_chi'].lower():
                print(f"{ma_lop:<8} {ma_sv:<10} {sv['ho_ten']:<30} {sv['ngay_sinh']:<12} {sv['gioi_tinh']:<10} {sv['dia_chi']:<20} {sv['khoa']:<15} {sv['tinh_trang']:<12} {sv['so_tiet_nghi']:<5}")
                tim_thay = True
    if not tim_thay:
        print("Không tìm thấy sinh viên nào theo địa chỉ yêu cầu.")
...
def xoa_lop():
    ...
    ma_lop = input("Nhập mã lớp cần xóa: ").strip()
...
if __name__ == "__main__":
    ...
    while True:
        print("""
            ...
            16. Hiển thị danh sách sinh viên đi học đầy đủ
            17. Thống kê số lượng sinh viên theo lớp
            18. Tìm sinh viên theo địa chỉ
            19. Xóa lớp :))
        ...
        if choice == '1':
            ...
            sinh_vien = hien_thi_danh_sach_sinh_vien_theo_ma_lop()
        elif choice == '11':
            sinh_vien = hien_thi_danh_sach_sinh_vien_theo_tinh_trang()
        elif choice == '12':
            sinh_vien = tim_Kiem_sinh_vien_theo_ma_sinh_vien()
        elif choice == '13':
            sinh_vien = tim_Kiem_sinh_vien_theo_ten()
        elif choice == '14':
            sinh_vien = hien_thi_danh_sach_sinh_vien_theo_tinh_trang_cam_thi()
        elif choice == '15':
            sinh_vien = hien_thi_sinh_vien_theo_so_tiet_nghi()
        elif choice == '16':
            sinh_vien = hien_thi_sinh_vien_di_hoc_day_du()
        elif choice == '17':
            sinh_vien = thong_ke_so_luong_theo_lop()
        elif choice == '18':
            sinh_vien = tim_sinh_vien_theo_dia_chi()
        elif choice == '19':
            sinh_vien = xoa_lop()
        elif choice == '20':
